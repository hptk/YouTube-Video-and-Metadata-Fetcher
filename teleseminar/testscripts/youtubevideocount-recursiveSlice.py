...
videoIDs = {}
pageToken = ""
publishedBefore = datetime.datetime.utcnow().replace(minute=0,second=0,microsecond=0)-datetime.timedelta(days=24*10)
publishedAfter = None
countAll = 0
pageAll = 0
countRequests = 0
    ...
    #    es.index(index="youtubetestindexerror",doc_type="SSLError",body=doc)
		...
		if "items" in jsonResult:
			...
			for item in jsonResult['items']:
				...
				es.index(index="youtubetestindex5",doc_type="video",id=item['id']['videoId'],body=doc)
