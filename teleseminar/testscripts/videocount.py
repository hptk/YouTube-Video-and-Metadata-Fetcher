...
pageToken = ""
publishedBefore = datetime.datetime.utcnow().replace(minute=0,second=0,microsecond=0)-datetime.timedelta(days=2)
publishedAfter = None
    ...
    while not exitFlag:
        ...
        if not workQueue.empty():
            ...
            if "items" in jsonResult:
                results += len(jsonResult['items'])
            if "nextPageToken" in jsonResult:
                while "nextPageToken" in jsonResult:
                    nextPage = jsonResult['nextPageToken']
                    jsonResult = json.loads(urllib2.urlopen(buildRequest(publishedAfter,publishedBefore,nextPage)).read())
                    results += len(jsonResult['items'])
                print str(publishedAfter)+"\t"+str(publishedBefore)+"\t"+str(results)
            else:
                print str(publishedAfter)+"\t"+str(publishedBefore)+"\t"+str(results)
